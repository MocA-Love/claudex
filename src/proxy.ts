...
const WEB_RESEARCH_POLICY = `Web research policy:
- Treat WebSearch as URL discovery only.
- Do not answer user-facing factual questions from WebSearch results alone.
- After using WebSearch for current or source-dependent information, use WebFetch on the 1-3 most relevant URLs before answering.
- Base the final answer on fetched page contents, preferring primary sources when available.
- If the user explicitly asks only for links or search results, you may stop after WebSearch.
- If WebFetch fails or the fetched pages do not contain the needed detail, say that explicitly instead of guessing.`;

function hasFunctionToolNamed(tools: Array<Record<string, unknown>>, name: string): boolean {
  const normalizedName = name.trim().toLowerCase();
  return tools.some(
    (tool) =>
      tool.type === "function" &&
      typeof tool.name === "string" &&
      tool.name.trim().toLowerCase() === normalizedName
  );
}

function appendWebResearchPolicyIfNeeded(parsed: JsonObject, tools: Array<Record<string, unknown>>): void {
  if (!hasFunctionToolNamed(tools, "WebSearch") || !hasFunctionToolNamed(tools, "WebFetch")) {
    return;
  }

  const existingInstructions = typeof parsed.instructions === "string" ? parsed.instructions.trim() : "";
  if (existingInstructions.includes("Web research policy:")) {
    return;
  }
...
function appendUniqueIncludeValues(target: JsonObject, values: string[]): void {
...
function adaptAnthropicMessagesRequestForResponses(
  ...
  const adapted: JsonObject = {
    ...
    input: toResponsesInput(parsed.messages),
    store: false,
  };
  ...
  const instructions =
    typeof parsed.instructions === "string" && parsed.instructions.trim().length > 0
      ...
      : extractInstructionsFromSystem(parsed.system);
  if (instructions) {
    adapted.instructions = instructions;
  }

  const mappedTools = mapAnthropicToolsToResponsesTools(parsed.tools);
  appendWorkspaceSummaryIfNeeded(adapted, mappedTools, options.workspaceSummary);
  appendWebResearchPolicyIfNeeded(adapted, mappedTools);
  if (mappedTools.length > 0) {
    adapted.tools = mappedTools;
  }

  if (Array.isArray(parsed.include)) {
    appendUniqueIncludeValues(
      adapted,
      parsed.include.filter((entry): entry is string => typeof entry === "string" && entry.trim().length > 0)
    );
  }

  if (mappedTools.some((tool) => tool.type === "web_search")) {
    appendUniqueIncludeValues(adapted, ["web_search_call.action.sources"]);
  }

  const mappedToolChoice = mapAnthropicToolChoiceToResponsesToolChoice(parsed.tool_choice);
  if (mappedToolChoice !== undefined) {
    adapted.tool_choice = mappedToolChoice;
  }
      ...
      if (method === "POST" && path === "/v1/messages") {
        ...
        if (options.upstreamWireApi === "responses") {
          await proxyResponsesAsAnthropic(
            ...
            upstreamOrigin,
            authState,
            adaptAnthropicMessagesRequestForResponses(parsed, options, isChatgptCodexOrigin(upstreamOrigin)),
            options
          );
          return;
        }
